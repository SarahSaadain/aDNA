...
def prepare():
    print("Preparing Bger for processing.")
    print("This script will prepare the folder structure and copy the files to the correct places.")
    print("It will also create mappings from folder names to lane numbers and from run IDs to sample names.")
    ...
    try:
        ...
        print(f"Found {len(raw_fastq_files)} raw fastq files.")

        # if no files are found, exit
        if len(raw_fastq_files) == 0:
            print("No raw fastq files found. Exiting.")
            return

        # read mapping of folder names to lane numbers
        print(f"Read mapping of folder names to lane numbers from {os.path.join(get_folder_path_species_resources(FOLDER_BGER), MAPPING_FILENAME_FOLDER_TO_LANE)}")
        map_folder_to_lane = get_mapping_folder_to_lane()
        
        # read mapping of run IDs to sample names
        print(f"Read mapping of run IDs to sample names from {os.path.join(get_folder_path_species_resources(FOLDER_BGER), MAPPING_FILENAME_RUNID_TO_NAME)}")
        ...
        for fastq_file_path in raw_fastq_files:
            ...
            print(f"Fastqfile: {filename}, folder name: {folder_name}, run ID: {runID}, Sample name: {sample_name}, lane number: {lane_number}, new filename: {new_filename}")
...
def move_file(source_path,target_path,new_filename=None):
    ...
    print(f"File moved from '{source_path}' to '{target_file_path}'")